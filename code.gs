...
function initSheetHeaders(sheet, name) {
  ...
  else if (name === "Comments")     headers = ["id","movie_id","user_id","user_name","content","timestamp","likes","edited"];
...
function doPost(e) {
  ...
  try {
    ...
    else if (action === "addComment")      result = addComment(body.movieId, body.userId, body.userName, body.content);
...
function getMovies() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    movies.push(movieRowToObj(headers, data[i]));
  }
...
function getAllComments() {
  var comments = sheetToObjects(getSheet("Comments")).map(function(c) {
    return {
      id: String(c.id), movie_id: String(c.movie_id),
      user_id: String(c.user_id), user_name: String(c.user_name),
      content: String(c.content), timestamp: String(c.timestamp),
...
function addComment(movieId, userId, userName, content) {
  var id  = generateId();
  var now = new Date().toISOString();
  getSheet("Comments").appendRow([id, movieId, userId, userName, content, now, 0, false]);
  return { success:true, comment: { id:id, movie_id:movieId, user_id:userId, user_name:userName, content:content, timestamp:now, likes:0, edited:false } };
}
