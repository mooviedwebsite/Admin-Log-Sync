...
// ── Admin auth: credentials are never stored in plain text ───────────────────
// These are SHA-256 hashes only. The real email/password are never in this file.
// To change credentials: hash new values with SHA-256 and replace these strings.
// Example (Node.js): require('crypto').createHash('sha256').update('yourpassword').digest('hex')
...
export const gasComments = {
  async getComments(movieId: string): Promise<Comment[]> {
    try { const data = await gasGet("getComments", { movieId }); return data.comments || []; } catch { return []; }
  },
  async getAllComments(): Promise<Comment[]> {
    try { const data = await gasGet("getAllComments"); return data.comments || []; } catch { return []; }
  },
  async addComment(
    movieId: string, userId: string, userName: string, content: string,
    replyTo?: string, replyToName?: string
