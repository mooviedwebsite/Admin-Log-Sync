...
export default function MovieBanner({
  movie, onPlay,
  bookmarked = false, onBookmark, showBookmark = false,
}: MovieBannerProps) {
      ...
      <div className="carousel-wrapper" style={{ cursor:"default" }}>
        <div className="carousel-track" style={{ transform:"translateX(0)" }}>
          <div className="movie-banner active">
            ...
            {/* Poster — via Cloudflare image proxy */}
            <div
            ...
            <div className="banner-content" style={{ paddingTop:"80px" }}>
              <Link
                ...
                style={{ display:"inline-flex", alignItems:"center", gap:"6px", color:"rgba(255,255,255,0.55)", fontSize:"13px", textDecoration:"none", marginBottom:"18px", transition:"color 0.2s" }}
              ...
              <div className="hb-badges">
                {movie.rating && (
                  <div className="hb-imdb-badge">
                    ...
                    <span className="hb-imdb-rating"><span className="hb-star">★</span>{movie.rating}</span>
                  </div>
                )}
                ...
                <div className="hb-info-item" style={{ marginLeft:"4px" }}>
                  ...
                  <span className="hb-info-value" style={{ fontSize:"12px" }}>{formatViews(movie.views)}</span>
                </div>
              </div>
              ...
              {movie.description && (
                <p className="hb-description" style={{ maxWidth:"700px" }}>{movie.description}</p>
              )}

              {movie.runtime && (
                <div className="hb-info-row">
                  <div className="hb-info-item"><span className="hb-info-label">Runtime:</span><span className="hb-info-value">{movie.runtime}</span></div>
                  <div className="hb-info-item"><span className="hb-info-label">Genre:</span><span className="hb-info-value">{movie.genre}</span></div>
                  {movie.year && <div className="hb-info-item"><span className="hb-info-label">Year:</span><span className="hb-info-value">{movie.year}</span></div>}
                </div>
              )}

              <div className="hb-action-btns">
                {/* Watch Now — calls onPlay → MoviePage handlePlay (with fade + toast) */}
                ...
                {movie.download_url && movie.download_url !== "#" && (
                  <a href={movie.download_url} className="hb-btn hb-btn-download" target="_blank" rel="noreferrer">
                ...
                {showBookmark && (
                  <button
                    ...
                    style={bookmarked ? { background:"rgba(255,215,0,0.15)", borderColor:"rgba(255,215,0,0.5)", color:"#FFD700" } : {}}
                  >
                    {bookmarked ? (
                      <><BookmarkCheck className="hb-btn-icon" />Saved</>
                    ) : (
                      <><Bookmark className="hb-btn-icon" />Save</>
                    )}
                  </button>
                )}
              </div>
